...
class MethodCallHandlerImpl implements Application.ActivityLifecycleCallbacks, InAppPurchaseApi {
        ...
        try {
            ...
            billingClient.queryProductDetailsAsync(
                    ...
                    (billingResult, queryProductDetailsResult) -> {
                        // In Billing Library 8.x, extract the list from QueryProductDetailsResult
    ...
    public @NonNull PlatformBillingResult launchBillingFlow(
        ...
        if (params.getOldProduct() != null
            ...
            if (params.getReplacementMode()
                    != REPLACEMENT_MODE_UNKNOWN_SUBSCRIPTION_UPGRADE_DOWNGRADE_POLICY) {
                // In Billing Library 8.x, setSubscriptionReplacementMode is deprecated but still available
                // Suppressed deprecation warning at method level
    ...
    public void queryPurchaseHistoryAsync(
        ...
        try {
            // Billing Library 8.x removed queryPurchaseHistoryAsync
            // This method is deprecated and should not be used
            // Return an error to indicate this API is no longer supported
            result.error(new FlutterError(
                    "UNSUPPORTED_API",
                    "queryPurchaseHistoryAsync has been removed in Billing Library 8.x. Use queryPurchasesAsync instead.",
                    null));
        } catch (RuntimeException e) {
            result.error(new FlutterError("error", e.getMessage(), Log.getStackTraceString(e)));
        }
